...
- VRKit Plugin [plugin.json5](plugin.json5)
- NPM Package [package.json](package.json)
...
capabilities. The required [esbuild.config.js](esbuild.config.js), which
...
> IMPORTANT: Make sure you use a newly generated UUID in [plugin.json5](plugin.json5)
> & the same UUID when publishing to [vrkit-plugin-manifest](https://github.com/vrkit-platform/vrkit-plugin-manifest)

These processes are taken care of via [GitHub Actions](.github/workflows/release.yml).
